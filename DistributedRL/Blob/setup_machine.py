...
with open('C:/prereq/log.txt', 'w') as f:
    ...
    # Configure AirSim to use the same json file
    if not os.path.isdir('D:\\Users\\explorationrl\\Documents\\AirSim'):
        do_command('mkdir D:\\Users\\explorationrl\\Documents\\AirSim')
    do_command('"C:\\Program Files (x86)\\Microsoft SDKs\\Azure\\AzCopy\\AzCopy.exe" /Source:https://explorationrlsa.blob.core.windows.net/prereq/settings.json /Dest:D:\\Users\\explorationrl\\Documents\\AirSim\\settings.json')
    do_command('call C:\\prereq\\mount.bat')
    
    # Download AirSim if it's not already on disk.
    if not os.path.isdir('D:\\AD_Cookbook_AirSim'):
        do_command('"C:\\Program Files (x86)\\Microsoft SDKs\\Azure\\AzCopy\\AzCopy.exe" /Source:https://explorationrlsa.blob.core.windows.net/prereq/AD_Cookbook_AirSim.7z /Dest:D:\\\\tmp.7z')
    ...
    do_command('schtasks.exe /create /xml Z:\\scripts_downpour\\run_airsim_on_user_login.xml /RU explorationrl /RP Erl53232 /tn StartAirsimIfAgent /IT')